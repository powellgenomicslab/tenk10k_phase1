...
if (file.exists(azimuth_file) == FALSE) {
  stop(paste0("Input file does not exist: ", azimuth_file))
  quit()
}
...
if (file.exists(hierscpred_file) == FALSE) {
  stop(paste0("Input file does not exist: ", hierscpred_file))
  quit()
}
...
# combine and save
df3 <- cbind(df1, df2)
print("Saving combined cell type predictions table...")
