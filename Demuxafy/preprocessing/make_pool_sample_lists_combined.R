...
# * BioHEART AND TOB *
# donor information (which BioHEART IDs are in which pool)

# NOTE:
# The input file "tenk10k_donor_pool_info.csv" can be generated with the below script.
...
get_n_donors_all_pools <- function(pool_list_suffix, pool_list_path, save = FALSE, pools_dir) {
    ...
    get_donors_in_libraries <- function(pool, pools_dir, save) {
        ...
        tenk10k_pool <- str_remove(pool, pattern = "_re$|_repeat$|a$|b$|_re1$")
