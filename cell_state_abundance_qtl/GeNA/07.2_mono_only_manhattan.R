...
# specify the SNP id's to label in the plot
snps_label <- c("12:69350234:C:A")
...
# Get summstats for just Mono
sumstats_all_ct <- "Monocyte" %>%
...
px <- 3000
ptsize <- px * (6.7 / 1250) # compute sensible ptsize based on pixel resolution
aspect_ratio <- 0.25
...
plot_data <- plot_data %>% mutate(
    label = fifelse(ID %in% snps_label, TRUE, FALSE)
)
...
all_ct_manhattan_alt_cols <- ggplot(data = plot_data, aes(x = BPcum, y = neg_log10_P)) +
    ...
    geom_text_repel(
        aes(label = ID),
        size = 6,
        data = plot_data %>% dplyr::filter(ID %in% snps_label),
        max.overlaps = Inf,
        # point.padding = 0.2,
        nudge_x = -5.5e8,
        nudge_y = 10,
...
all_ct_manhattan_alt_cols %>% ggsave(
    filename = glue("/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/csa_qtl/figures/{resolution}/combined_plots/split_Monocyte_manhattan_alternative_colours.pdf"),
    width = 10, height = 4,
)
