...
@click.option("--celltype", help="Name of the cell type")
@click.option(
...
def main(
    ...
    if re.search(pattern, gwas_file_path):
        gwas_chr = int(
            re.search(pattern, gwas_file_path)
            .group()
            .replace("_snp_str_gwas_results_hg38_chr", "")
        )
        if gwas_chr != chr:
            logging.warning(
                f"GWAS file for different chr than target SNP, skipping...."
            )
            return
    ...
    gwas = gwas.loc[
        (gwas["chromosome"] == f"chr{chr}")
        & (gwas["position"] > pos - cis_window)
        & (gwas["position"] < pos + cis_window),
        :,
    ]
    ...
    if sum((gwas["chromosome"] == f"chr{chr}")) == 0:
        logging.warning(
            f"No SNPs for chr{chr} in gwas file: {os.path.basename(gwas_file_path)} skipping...."
        )
    elif gwas["p_value"].min() > gwas_significance_threshold:
        logging.warning(
            f"No significant SNP GWAS data in the cis-window around chr{chr} {pos}: skipping...."
        )
    elif gwas.empty:
        logging.warning(
            f"No SNPs present in cis window {cis_window} around lead SNP at chr{chr} {pos}: skipping...."
        )
    else:
        logging.info(
            f"Running coloc for lead SNP at chr{chr} {pos} and gwas: {gwas_file_path}"
        )

        # run coloc
        coloc_runner(
            gwas=gwas,
            gwas_pheno_name=gwas_pheno_name,
