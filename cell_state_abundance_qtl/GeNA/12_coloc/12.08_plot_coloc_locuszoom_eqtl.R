...
# plot all colocalised loci for each cell type
for (celltype in celltypes) {
    ...
    # gwas_file_list <- c("mono.ma", "mono_p.ma")
    # gwas_file <- gwas_file_list[1]
    # gwas_pheno_name <- gwas_file_list[1] %>% str_remove(".ma")

    # # TODO: function to read GWAS and re-format it for locus zoom
    # read_gwas <- function(gwas_dir, gwas_file) {
    #     gwas_path <- glue("{gwas_dir}/{gwas_file}")
    #     gwas <- readr::read_tsv(gwas_path,
    #         # col_names = c("chromosome", "position", "varbeta", "beta", "snp", "MAF", "p_value"),
    #         col_types = cols(
    ...
    #         )
    #     ) %>%
    #         mutate(chromosome = as.integer(str_remove(chromosome, "chr")))
    #     return(gwas)
    # }

    # gwas_summstats <- read_gwas(gwas_dir = gwas_dir, gwas_file = gwas_file)
    # setDT(gwas_summstats)
    ...
    plot_gene_locuszoom <- function(lead_snp, gene_eqtl, sumstats_csaqtl, sumstats_gwas, gwas_pheno_name, sumstats_eqtl, celltype, minor_cell_types = NULL, ct_pval_thresholds, tenk_color_pal, resolution = "major_cell_types", analysis_name = "no_expr_pc_covars") {
        ...
        fixed_phenotype_csaqtl_file <- glue("/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/csa_qtl/output/coloc/plink_fixed_pheno/{celltype}/gwas_local_{lead_snp}_cis.spheno_{lead_snp}.glm.linear")
        csaqtl_fixed_phenotype <- fread(fixed_phenotype_csaqtl_file, sep = "\t", header = TRUE) %>%
            mutate(P = fifelse(as.numeric(P) == 0, .Machine$double.xmin * .Machine$double.eps, as.numeric(P)))

        loc_csaqtl_fixed <- locus(
            data = csaqtl_fixed_phenotype,
            # gene = gene,
            seqname = chr,
            xrange = c(pos - 1e5, pos + 1e5),
            flank = 1e5,
        ...
        locus_plot_list[[glue("{celltype} csaQTL fixed phenotype")]] <- gg_scatter(
            loc_csaqtl_fixed,
            labels = "index",
            ylab = paste0(" GeNA fixed phenotype ", "-log10(P) "),
            scheme = c("lightgrey", ct_col, "red"),
            xticks = FALSE
        ) + annotate("text", x = -Inf, y = 5e-8, col = "grey", label = glue("  P = 5e-8\n", .trim = FALSE), vjust = 0, hjust = 0) +
            annotate("text", x = Inf, y = Inf, col = "black", label = glue("{str_replace(celltype, '_', ' ')}  "), vjust = 1, hjust = 1)

        # # if gwas data provided, plot gwas too
        # locus_gwas <- locus(
        #     data = sumstats_gwas,
        #     gene = gene,
        #     flank = 1e5,
        ...
        # locus_plot_list[[glue("{gwas_pheno_name} GWAS")]] <- gg_scatter(
        #     locus_gwas,
        #     labels = "index",
        #     ylab = paste0(gwas_pheno_name, " GWAS ", "-log10(P) "),
        #     scheme = c("lightgrey", "darkgrey", "red"),
        #     xticks = FALSE
        # ) + annotate("text", x = -Inf, y = 5e-8, col = "grey", label = glue("  P = 5e-8\n", .trim = FALSE), vjust = 0, hjust = 0) # +
        # # annotate("text", x = Inf, y = Inf, col = "black", label = glue("{str_replace(celltype, '_', ' ')}  "), vjust = 1, hjust = 1)
