# script from tensorqtl https://github.com/broadinstitute/tensorqtl/blob/master/tensorqtl/pgen.py#L32
...
def read_dosages_range(
    pgen_path, start_idx, end_idx, sample_subset=None, dtype=np.float32
):
    """
...
class PgenReader(object):
    ...
    def read_region(
        self, region, start_pos=None, end_pos=None, impute_mean=True, dtype=np.float32
    ):
    ...
    def read_dosages_region(
        self, region, start_pos=None, end_pos=None, dtype=np.float32
    ):
        r = self.get_range(region, start_pos, end_pos)
        if len(r) > 0:
