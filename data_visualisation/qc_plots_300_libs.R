...
tenk10k_metadata <- fread("/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/scanpy/output/integrated_objects/300_libraries/300_libraries_cell_metadata_subset_pre_filtering.csv")
# tenk10k_metadata <- tenk10k_metadata[, !duplicated(colnames(tenk10k_metadata)), with = FALSE] # oops. remove the duplicated columns

tenk10k_metadata$wg2_scpred_prediction %>% table()
...
tenk10k_metadata <- tenk10k_metadata %>%
    filter(!wg2_scpred_prediction %in% c("Platelet", "Eryth", "Doublet"))
...
hist_list %>%
    ...
    ggsave(filename = "/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/figures/qc/300_libs/qc_histograms.png", height = 10, width = 10)
...
density_list %>%
    ...
    ggsave(filename = "/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/figures/qc/300_libs/qc_density.png", height = 10, width = 8)

vln_list <- metrics_to_plot %>%
    purrr::map(\(metric) qc_vln(df = tenk10k_metadata, metric = metric, group_col = "wg2_scpred_prediction"))

vln_list %>%
    patchwork::wrap_plots(ncol = 2, guides = "collect") %>%
    ggsave(filename = "/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/figures/qc/300_libs/qc_vln_by_cluster_plots.png", height = 15, width = 15)
...
n_genes_hist_zoomed <- qc_hist(df = tenk10k_metadata, metric = "n_genes") +
    geom_vline(xintercept = 600, colour = "red", linetype = "dashed") +
...
hist_list_zoomed %>%
    patchwork::wrap_plots(ncol = 2, guides = "collect") %>%
    ggsave(filename = "/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/figures/qc/300_libs/qc_histograms_zoomed.png", height = 15, width = 15)
...
total_counts_hist_zoomed_split <- tenk10k_metadata %>%
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
    scale_x_continuous(limits = c(0, 4000)) +
    facet_wrap(~wg2_scpred_prediction, ncol = 2)

total_counts_hist_zoomed_split %>%
    ggsave(filename = "/directflow/SCCGGroupShare/projects/blabow/tenk10k_phase1/data_processing/figures/qc/300_libs/qc_histograms_zoomed_split.png", height = 25, width = 15)
...
tenk10k_metadata <- tenk10k_metadata %>% mutate(
    ...
    # platelets_erythrocytes_doublets = if_else(wg2_scpred_prediction %in% c("Platelet", "Eryth", "Doublet"), TRUE, FALSE),
    below_600_n_genes_or_below_800_total_counts = if_else(n_genes < 600 | total_counts < 800, "remove", "keep")
)

filtered <- tenk10k_metadata %>%
    filter(!wg2_scpred_prediction %in% c("Platelet", "Eryth", "Doublet")) %>%
    filter(below_600_n_genes_or_below_800_total_counts == "keep")


table(filtered$wg2_scpred_prediction)

table(tenk10k_metadata$below_600_n_genes_or_below_800_total_counts)

tenk10k_metadata$sequencing_library
...
colSums(is.na())
is.na(tenk10k_metadata$cellbender_background_fraction)

tenk10k_metadata %>%
    filter(is.na(cellbender_background_fraction)) %>%
    pull(sequencing_library) %>%
    table()

missing_cb <- tenk10k_metadata %>%
    filter(is.na(cellbender_background_fraction)) %>%
    pull(sequencing_library) %>%
    unique()

missing_cb_ind <- tenk10k_metadata %>%
    filter(is.na(cellbender_background_fraction)) %>%
    pull(individual) %>%
    unique()

tenk10k_metadata %>%
    filter(sequencing_library %in% missing_cb) %>%
    select(cellbender_background_fraction) %>%
    is.na() %>%
    table()

tenk10k_metadata %>%
    filter(individual %in% missing_cb_ind) %>%
    select(cellbender_background_fraction) %>%
    is.na() %>%
    table()

tenk10k_metadata %>%
    filter(individual == "CPG305672") %>%
    select(cellbender_background_fraction) %>%
    is.na() %>%
    table()
